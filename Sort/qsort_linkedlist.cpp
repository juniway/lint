...
Following is C++ implementation for same. The important things about implementation are, it changes pointers rather swapping data 
and time complexity is same as the implementation for Doubly Linked List.
...
After pivot is placed at correct position, we find tail node of left side (list before pivot) 
                       ...
                       struct node **newHead, struct node **newEnd)
{
    ...
    while (cur != pivot){
        if (cur->data < pivot->data){
            // Initially the first node that has a value less than the pivot 
...
I have simpler and easy to understand code:
...
now in while loop as we never get a node which is less than pivot node(in terms of data values) , 
therefore, it never goes in the 'if' block and hence *newhead=NULL. 
It always goes in the 'else' condition. So after this loop, the linked list becomes
