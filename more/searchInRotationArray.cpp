...
Use divide and conquer binary search. This is O(log(N)) no larger than the binary search O(log(N)). For each subarray check if the array is sorted. If sorted use classic binary search e.g
...
int main(){
	int numArr[12] = {15, 16, 19, 20, 25, 1, 3, 4, 5, 7, 10, 14};

	int index = searchInRotationArray(numArr, 14, 0, 11);
	if(index != -1)
