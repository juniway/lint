...
Given an array A, write a function to move all 0's to the end of it while maintaining
the relative order of the non-zero elements.

For example, given A = [0, 1, 0, 3, 12], after calling your function, A should be [1, 3, 12, 0, 0].
Note:
...
public:
    void moveZeroes(vector<int>& A) {
        ...
        for(int zero_index = 0, none_zero_index = 0;
            none_zero_index < n && zero_index < n;
        ) {
            if(A[zero_index] != 0) {
                ++zero_index;
